...
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n'
            + 'FN: SADZINMAKER\n'
            + 'ORG:Owner SADZINMAKER;\n'
            + 'TEL;type=CELL;type=VOICE;waid=5514998360726:+55 14 99836-0726\n'
            + 'END:VCARD'

prefix = "!"
name = "~ SADZINN  | MAKER"
rdaftar = "OBRIGADO POR SE REGISTAR PARA SER UM SADBOY (☞ ಠ_ಠ)☞"
rmenu = "SADZINN DOMINA GARAI:("
limitt = 10
ban = []
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					userB: `salve Mano ${pushname2} voce nn esta cadastrado no banco de dados do SADZIN MAKER digite ${prefix}daftar`,
		...
		case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('vocês já são amigos do SADZIN MAKER:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = '```Lista Banido``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```Lista de Premium SADZIN MAKER``` :\n'
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Não registrado no banco de dados *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[SADZIN MAKER]','aqua'), 'Comando não registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
