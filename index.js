...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					benned: '*desculpe, seu número foi banido, por favor contate o dono para remover seu ban*',
					ownerG: '*desculpe, este comando só pode ser usado pelo proprietario do grupo!*',
					ownerB: '*desculpe este comando so pode ser usado pelo proprietario do bot!* ',
					premium: '*desculpe, este comando e especificamente para usuários premium!!*',
					userB: `salve Mano ${pushname2} voce nn esta cadastrado no banco de dados do bot do sadzinmaker digite ${prefix}daftar`,
					admin: '*desculpe este comando so pode ser usado por admin do grupo!*',
					Badmin: '*desculpe este comando so pode ser usado se o bot for admin do grupo!*'
				}
			}
		...
		case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('vocês já são amigos do sadzinmaker ᕙ( ͡◉ ͜ ʖ ͡◉)ᕗ')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (isBanned) return reply(mess.only.benned)
                ...
                myDays = ['Domigo','Segunda-feira','terça feira','Quarta feira','Quinta feira','Jum at','sábado'];
                var tgl = new Date();
                var day = tgl.getDate()
                  bulan = tgl.getMonth()
				...
				ben = '```Lista Banido``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```Lista de Premium do bot do sadzinmaker``` :\n'
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break  
				...
				case 'owner':
                 ...
                 client.sendMessage(from, 'Salva o número do meu criador ae fdp:)',text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
				if (isBanned) return reply(mess.only.benned)    
...
//akhir kreator
		    ...
		    case 'map':
				...
				case 'clearall':
					...
					reply(`\`\`\`Sucesso deleta todo o chat SADZIN MAKER\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *SADZIN MAKER BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Transmissão de sucesso')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Não registrado no banco de dados *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ SADZIN ]','aqua'), 'Comando não registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
